...
def crop_img(src, top_left_x, top_left_y, crop_w, crop_h):
    ...
    if row_max > rows or col_max > cols:
        print(
            "crop size err: src->%dx%d,crop->top_left(%d,%d) %dx%d"
...
def crop_imgs(
    img, label, crop_type="RANDOM_CROP", crop_n=1, dsize=(0, 0), random_wh=False
):
    """
    Args：
        imgs_dir: 待放缩图片
    ...
    croped_label = crop_img(
        label, crop_top_left_x, crop_top_left_y, crop_imgw, crop_imgh
    )
...
def random_color_distort(
    img, label, brightness_delta=32, hue_vari=18, sat_vari=0.5, val_vari=0.5
):
    """
