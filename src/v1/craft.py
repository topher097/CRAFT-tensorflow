...
def train(train=True):
    ...
    include = [
        ...
        "vgg_16/conv2/conv2_2vgg_16/conv3/conv3_1",
    ...
    learning_rate = tf.train.piecewise_constant(
        global_step, boundaries=boundaries, values=learning_rate
    )
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for e in range(epoch):
            ...
            for i in range(data_len // batch_size):
                ...
                _, loss_f0, learning_rate0, global_step0 = sess.run(
                    [train_step, loss, learning_rate, global_step],
                    feed_dict={x: image, y: label},
                )
                print(
                    "\rstep: %2d   learning_rate: %4g   total_loss: %4g"
                    % (global_step0, learning_rate0, loss_f0),
                    end="",
                )
                loss_t += loss_f0
                if global_step0 % 100 == 0:
                    ...
                    if global_step0 % 1000 == 0:
                        saver.save(
                            sess, "/home/user4/ysx/demo/CRAFT_%d.ckpt" % (global_step0)
                        )
