...
def Random_rot(img, bbox):
    ...
    rx0, ry0 = w / 2, h / 2
    bbox[0, :, :] = (
        (bbox[0, :, :] - rx0) * math.cos(math.radians(angle))
        - (bbox[1, :, :] - ry0) * math.sin(math.radians(angle))
        + rx0
    )
    bbox[1, :, :] = (
        (bbox[0, :, :] - rx0) * math.sin(math.radians(angle))
        + (bbox[1, :, :] - ry0) * math.cos(math.radians(angle))
        + ry0
    )
    # bbox[bbox<0] = 0
    # bbox[0,:,:][bbox[0,:,:]>h] = h
