...
def MSE_OHEM_Loss(output_imgs, target_imgs):
    ...
    for i in range(batch_size):
        ...
        positive_loss_m = tf.reduce_sum(positive_loss) / tf.cast(
            num_positive, dtype=tf.float32
        )
        ...
        k = tf.cond(
            (k + num_positive) > num_all,
            lambda: tf.cast((num_all - num_positive), dtype=tf.int32),
            lambda: k,
        )
        ...
        res = tf.cond(
            k < 10,
            lambda: tf.reduce_mean(sample_loss),
            lambda: positive_loss_m
            + tf.reduce_sum(nagative_loss_topk) / tf.cast(k, dtype=tf.float32),
        )
...
if __name__ == "__main__":
    ...
    target_imgs = tf.constant([[1.1, 2.1], [3.0, 4.1]], dtype=tf.float32)
