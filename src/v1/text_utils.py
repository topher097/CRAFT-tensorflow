...
def getDetBoxes_core(
    textmap, linkmap, text_threshold, link_threshold, low_text, s=True
):
    # prepare data
...
def getDetBoxes(textmap, linkmap, text_threshold, link_threshold, low_text, s=True):
    boxes, labels, mapper = getDetBoxes_core(
        textmap, linkmap, text_threshold, link_threshold, low_text, s
    )
...
def get_result_img(
    ...
    boxes = getDetBoxes(
        score_text, score_link, text_threshold, link_threshold, low_text, s=False
    )
    boxes = adjustResultCoordinates(boxes, ratio_w, ratio_h)
    file_utils.saveResult(
        "/home/user4/ysx/text_image/text_image_char.jpg",
        image,
        boxes,
        dirname="/home/user4/ysx/text_image/",
    )
    boxes = getDetBoxes(
        score_text, score_link, text_threshold, link_threshold, low_text, s=True
    )
    boxes = adjustResultCoordinates(boxes, ratio_w, ratio_h)
    file_utils.saveResult(
        "/home/user4/ysx/text_image/text_image_word.jpg",
        image,
        boxes,
        dirname="/home/user4/ysx/text_image/",
    )
